...
bool shadowDiscardNotMyDeltaFlag = true;
	...
	if(isAckForMyThingName(topicName)) {
		...
		if(extractVersionNumber(shadowRxBuf, pJsonHandler, tokenCount, &tempVersionNumber)) {
			if(tempVersionNumber > shadowJsonVersionNum) {
				/*
				if need update version number in ack callback?
				modified by tom.zhang
				*/
...
static void shadow_delta_callback(AWS_IoT_Client *pClient, char *topicName,
	...
	uint32_t dataLength;
	uint32_t tempVersionNumber = 0;
	uint8_t extractedClientToken[MAX_SIZE_CLIENT_ID_WITH_SEQUENCE];
	...
	//modified by tom.zhang
	if(shadowDiscardNotMyDeltaFlag){
		if(extractClientToken(shadowRxBuf,extractedClientToken)){
			uint8_t* pClientId = NULL;
			uint16_t len;
			
			pClientId = pClient->clientData.options.pClientID;
			len = pClient->clientData.options.clientIDLen;

			//"GDC_FFFFFF" for OTA upgrade all GDC firmware
			if((strncmp(pClientId,extractedClientToken,len) != 0) && 
				(strncmp("GDC_FFFFFF",extractedClientToken,len) != 0)){
				return;
			}
		}
	}
